...
import { HealthViewProvider, TasksViewProvider, SignalsViewProvider } from "./views";

let runner: AlfredRunner;
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  runner = new AlfredRunner(workspaceRoot, context);
  ...
  // ── Tree View Providers ────────────────────────────────────────
  const healthProvider = new HealthViewProvider(runner);
  const tasksProvider = new TasksViewProvider(runner);
  const signalsProvider = new SignalsViewProvider(runner);

  context.subscriptions.push(
    vscode.window.registerTreeDataProvider("alfred.healthView", healthProvider),
    vscode.window.registerTreeDataProvider("alfred.tasksView", tasksProvider),
    vscode.window.registerTreeDataProvider("alfred.signalsView", signalsProvider)
  );

  // ── Commands ───────────────────────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand("alfred.scan", async () => {
      await runWithProgress("Scanning repository...", async () => {
        ...
        vscode.window.showInformationMessage(
          `Alfred scan complete: ${result.file_count} files, ` +
    ...
    vscode.commands.registerCommand("alfred.runCycle", async () => {
      await runWithProgress("Running Alfred cycle (dry-run)...", async () => {
        ...
        vscode.window.showInformationMessage(
          `Alfred cycle complete: Health ${eval_.health_score}/100 (${eval_.verdict})`
        );
      });
    }),
    ...
    vscode.commands.registerCommand("alfred.runCycleLive", async () => {
      const confirm = await vscode.window.showWarningMessage(
        "Run Alfred in LIVE mode? This will execute real commands.",
        { modal: true },
        "Run Live"
      );
      if (confirm !== "Run Live") { return; }
      ...
      await runWithProgress("Running Alfred cycle (LIVE)...", async () => {
        ...
        const eval_ = result.evaluation;
        vscode.window.showInformationMessage(
          `Alfred LIVE cycle complete: Health ${eval_.health_score}/100 (${eval_.verdict})`
        );
      });
    }),
    ...
    vscode.commands.registerCommand("alfred.autofix", async () => {
      const confirm = await vscode.window.showWarningMessage(
        "Run Alfred auto-fix? This will modify source files.",
        { modal: true },
        "Auto-Fix"
      );
      if (confirm !== "Auto-Fix") { return; }
      ...
      await runWithProgress("Running auto-fix...", async () => {
        ...
        signalsProvider.refresh();
        vscode.window.showInformationMessage(
          `Alfred auto-fix: ${result.total_fixes} fixes applied`
        );
      });
    }),
    ...
    vscode.commands.registerCommand("alfred.gatesSweep", async () => {
      await runWithProgress("Running verification gates...", async () => {
        ...
        const passed = result.filter((r: any) => r.status === "done").length;
        const total = result.length;
        vscode.window.showInformationMessage(
          `Alfred gates: ${passed}/${total} passed`
        );
      });
    }),
    ...
    vscode.commands.registerCommand("alfred.showMemory", async () => {
      const memoryPath = vscode.Uri.file(
        `${workspaceRoot}/agents/alfred/state/memory.json`
      );
      try {
...
function computeHealth(scan: any): number {
  let score = 100;
...
function updateStatusBar(scan: any): void {
...
function generateHealthHtml(score: number, scan: any): string {
