...
import {
  getDebugEntries,
  getDebugEntriesByType,
  subscribeDebugBuffer,
  clearDebugBuffer,
  type DebugEntry,
} from '../../src/services/debugBuffer';
...
export default function DebugPanel() {
  ...
  const [syncSnapshot, setSyncSnapshot] = useState<SyncSnapshot>({ pendingCount: 0, lastSyncChannel: null, lastSyncTimestamp: null });
  ...
  const loadData = useCallback(async () => {
    try {
      // User state
      const { getCachedUserState } = await import('../../src/engines/UserStateEngine');
      const us = getCachedUserState();
      setUserState(us ? {
        consistencyScore: us.consistencyScore,
        engagementLevel: us.engagementLevel,
        fatigueTier: us.fatigueTier,
        churnRisk: us.churnRisk,
        daysSinceLastWorkout: us.daysSinceLastWorkout,
        streak: us.streak,
      } : null);
    ...
    try {
      ...
      setRecentWorkouts(sessions.map((s: any) => ({
        id: s.id,
    ...
    try {
      // Sync snapshot
      const { getPendingCount } = await import('../../src/services/offlineQueue');
      const count = await getPendingCount();
      const { dataSync } = await import('../../src/services/dataSyncService');
      const lastEvent = dataSync.getLastEvent('workout_completed');
      setSyncSnapshot({
        pendingCount: count,
        lastSyncChannel: lastEvent?.channel ?? null,
        lastSyncTimestamp: lastEvent?.timestamp ?? null,
      });
    } catch {
  ...
  // Filtered entries
  const eventEntries = entries.filter((e) => e.type === 'event').slice(-20).reverse();
  const navEntries = entries.filter((e) => e.type === 'navigation').slice(-10).reverse();
    ...
    <View style={[styles.container, { backgroundColor: bg, paddingTop: insets.top }]}>
      ...
      <View style={[styles.header, { borderBottomColor: theme.colors.border }]}>
        ...
        <TouchableOpacity onPress={() => clearDebugBuffer()} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
          <ThemedText variant="caption" style={{ color: error }}>CLEAR</ThemedText>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        >
          {userState ? (
            <>
              ...
              <Row label="Fatigue Tier" value={userState.fatigueTier ?? '—'} theme={theme}
                valueColor={userState.fatigueTier === 'HIGH' ? warning : undefined} />
              <Row label="Churn Risk" value={userState.churnRisk ? 'YES' : 'NO'} theme={theme}
            ...
            <ThemedText variant="caption" color="muted">No user state loaded</ThemedText>
          )}
        </SectionCard>
        ...
        >
          ...
          {recentWorkouts.length === 0 ? (
            <ThemedText variant="caption" color="muted">No workouts found</ThemedText>
          ) : (
        ...
        >
          ...
          {eventEntries.length === 0 ? (
            <ThemedText variant="caption" color="muted">No events yet</ThemedText>
          ) : (
            eventEntries.map((e) => (
        ...
        >
          ...
          {entries.filter((e) => e.type === 'network').slice(-5).reverse().map((e) => (
        ...
        >
          <Row label="Queue Length" value={String(syncSnapshot.pendingCount)} theme={theme}
          ...
          <Row label="Last Sync At" value={syncSnapshot.lastSyncTimestamp ? fmt(syncSnapshot.lastSyncTimestamp) : '—'} theme={theme} />
          {/* Sync debug entries */}
          {entries.filter((e) => e.type === 'sync').slice(-5).reverse().map((e) => (
            <View key={e.id} style={styles.miniRow}>
              <ThemedText variant="caption" style={{ color: text, flex: 1 }} numberOfLines={1}>
                {e.label}
              </ThemedText>
        ...
        >
          ...
          {navEntries.length === 0 ? (
            <ThemedText variant="caption" color="muted">No navigation events yet</ThemedText>
          ) : (
            navEntries.map((e) => (
              <View key={e.id} style={styles.miniRow}>
                <ThemedText variant="caption" style={{ color: text, flex: 1 }} numberOfLines={1}>
        ...
        >
          {entries.filter((e) => e.type === 'interaction').slice(-10).reverse().map((e) => (
            <View key={e.id} style={styles.miniRow}>
              <ThemedText variant="caption" style={{ color: text, flex: 1 }} numberOfLines={1}>
                {e.label}
              </ThemedText>
          ...
          {entries.filter((e) => e.type === 'interaction').length === 0 && (
            <ThemedText variant="caption" color="muted">No interactions yet</ThemedText>
          )}
        </SectionCard>
      </ScrollView>
    </View>
...
function SectionCard({ title, icon, color, theme, expanded, onToggle, children }: {
  title: string;
  icon: string;
  color: string;
  theme: any;
  ...
  return (
    <View style={[styles.card, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
      <TouchableOpacity style={styles.cardHeader} onPress={onToggle} activeOpacity={0.7}>
        ...
        <ThemedText variant="bodySmall" weight="700" style={{ color: theme.colors.text, flex: 1, marginLeft: spacing[2] }}>
          {title}
        </ThemedText>
        <MaterialCommunityIcons name={isOpen ? 'chevron-up' : 'chevron-down'} size={18} color={theme.colors.textMuted} />
      </TouchableOpacity>
...
function Row({ label, value, theme, valueColor }: {
  label: string; value: string; theme: any; valueColor?: string;
}) {
  return (
    <View style={styles.row}>
      <ThemedText variant="caption" style={{ color: theme.colors.textMuted, flex: 1 }}>{label}</ThemedText>
      <ThemedText variant="caption" weight="600" style={{ color: valueColor ?? theme.colors.text }}>{value}</ThemedText>
    </View>
  );
}
