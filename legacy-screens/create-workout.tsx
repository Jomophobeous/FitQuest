  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1, marginLeft: spacing[3] }}>
        <Text style={{ color: colors.text, fontWeight: '600', fontSize: typography.sizes.bodySmall }}>{item.name}</Text>
        <Text style={{ color: colors.textMuted, fontSize: typography.sizes.caption, marginTop: spacing[0.5] }}>
          {item.difficulty} • {item.primary_muscles.slice(0, 2).join(', ')}
        </Text>
      </View>
      <View style={[styles.diffBadge, { backgroundColor: colors.surface }]}>
        <Text style={{ color: colors.textMuted, fontSize: typography.sizes.xs, textTransform: 'uppercase' }}>
          ...
          {selected.map((item, index) => (
            <Card key={item.exercise.id} style={styles.configCard}>
              ...
              >
                ...
                />
                <Text style={{ color: theme.colors.accent, fontSize: typography.sizes.label, fontWeight: '600', marginLeft: spacing[1.5] }}>
                ...
                >
                  {item.exercise.instructions.map((instruction, idx) => (
                    <View key={idx} style={styles.instructionStep}>
                      <Text style={{ color: theme.colors.accent, fontSize: typography.sizes.caption, fontWeight: '700', width: 20 }}>
                        {idx + 1}.
                      </Text>
                      <Text style={{ color: theme.colors.textSecondary, fontSize: typography.sizes.label, flex: 1, lineHeight: 18 }}>
                        {instruction}
                      </Text>
                    </View>
                  ))}
                  ...
                  >
                    ...
                    <Text style={{ color: theme.colors.accent, fontSize: typography.sizes.caption, fontWeight: '600', marginLeft: spacing[1.5] }}>
                      Read Aloud
                    </Text>
                  </TouchableOpacity>
                </View>
            ...
            <Card key={item.exercise.id} style={styles.previewExercise}>
              <View style={styles.previewRow}>
                <View style={[styles.orderBadge, { backgroundColor: theme.colors.accent }]}>
                  <Text style={{ color: theme.colors.text, fontWeight: '700', fontSize: typography.sizes.caption }}>{index + 1}</Text>
                </View>
                <ExerciseImage
                  exerciseId={item.exercise.id}
                  category={item.exercise.category}
