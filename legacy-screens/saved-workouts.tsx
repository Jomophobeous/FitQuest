      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.background }]}>
        ...
        {vm.loading ? (
          <Animated.View entering={FadeIn.duration(150)} style={styles.loadingContainer}>
            ...
            <Text style={[styles.loadingText, { color: theme.colors.textMuted }]}>{t('savedWorkouts.loading') || 'Loading workouts…'}</Text>
          </Animated.View>
        ) : vm.loadError ? (
          <View style={[styles.loadingContainer, { alignItems: 'center' }]}>
            <MaterialCommunityIcons name="alert-circle-outline" size={48} color={theme.colors.error} />
            <Text style={[styles.loadingText, { color: theme.colors.error, textAlign: 'center', marginTop: spacing[4] }]}>{vm.loadError}</Text>
