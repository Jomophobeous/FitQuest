...
export default function WorkoutScreen() {
  ...
  useEffect(() => {
    if (sessionId && status === 'idle' && !workout) {
      ...
      const t = setTimeout(() => router.replace('/fitquest' as any), 200); // debounce
  ...
  const handleComplete = useCallback(async () => {
    ...
    setTimeout(() => setShowCompleteBadge(false), 1300); // animation
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        <View style={styles.centeredContent}>
          ...
          <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>{t('workout.generating') ?? 'Generating workout…'}</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
        <View style={styles.centeredContent}>
          ...
          <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>{t('common.error') ?? 'Something went wrong'}</Text>
          <Text style={[styles.emptySub, { color: theme.colors.textMuted }]}>{error}</Text>
                  ...
                  {exerciseTimer.state === 'running' && (
                    <View style={styles.exerciseTimerWrap}>
                      <ProgressRing
                        ...
                        strokeWidth={8}
                      >
                        <Text style={[styles.exerciseTimerText, { color: theme.colors.text, fontSize: typography.sizes.h1 }]}>
...
const styles = StyleSheet.create({
  ...
  diffBadge: { paddingHorizontal: spacing[2.5], paddingVertical: spacing[1], borderRadius: 8 },
  ...
  targetPill: { paddingHorizontal: spacing[2.5], paddingVertical: spacing[1], borderRadius: 8 },
