 ...
 */
function getThemedColor(color: string, themeMode: ThemeMode, themeAccent: string, themeAccentDark: string): string {
  if (themeMode !== 'blackGold') return color;
  // Map green hex variants → theme accent for blackGold theme
  const greenVariants = [themeAccent.toLowerCase(), themeAccentDark.toLowerCase()];
  if (greenVariants.includes(color.toLowerCase())) {
    return themeAccent;
  }
  return color;
}
...
export function RankBadge({ level, size = 'md' }: RankBadgeProps) {
  ...
  const rankColor = getThemedColor(currentRank.color, mode, theme.colors.accent, theme.colors.accentDark);
...
export function RankCard({ level, totalXP, showQuote = true }: RankCardProps) {
  ...
  const rankColor = getThemedColor(currentRank.color, mode, theme.colors.accent, theme.colors.accentDark);
  const tierColor = getThemedColor(tier.color, mode, theme.colors.accent, theme.colors.accentDark);
  ...
  return (
    <View style={styles.milestoneList}>
      {visibleRanks.map((milestone, i) => {
        ...
        const milestoneColor = getThemedColor(milestone.color, mode, theme.colors.accent, theme.colors.accentDark);
