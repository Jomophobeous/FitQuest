...
export class SleepAnalysisEngine {
  ...
  async recordManualSession(
    ...
    const efficiency = totalDurationMs > 0 ? Math.round((actualSleepMs / totalDurationMs) * 100) : 0;
  ...
  private estimateStages(actualSleepMs: number, awakenings: number): SleepStageBlock[] {
    ...
    for (let cycle = 0; cycle < numCycles; cycle++) {
      ...
      const deepFraction = Math.max(0.05, 0.3 - cycle * 0.05);
  ...
  private estimateStagesFromMovement(
    ...
    for (let offset = 0; offset < actualSleepMs; offset += windowMs) {
      ...
      const windowData = movementData.filter((d) => d.timestamp >= windowStart && d.timestamp < windowEnd);

      const avgIntensity =
        windowData.length > 0 ? windowData.reduce((a, b) => a + b.intensity, 0) / windowData.length : 0;
  ...
  private estimateSleepLatency(movementData: Array<{ timestamp: number; intensity: number }>, bedtime: number): number {
  ...
  private calculateQualityScore(params: {
    ...
    const deepMs = params.stages.filter((s) => s.stage === 'DEEP').reduce((a, b) => a + b.durationMs, 0);
    const deepRatio = deepMs / params.actualSleepMs;
    if (deepRatio < 0.1) score -= 10;
    else if (deepRatio < 0.15) score -= 5;

    // REM ratio (0-10 points)
    const remMs = params.stages.filter((s) => s.stage === 'REM').reduce((a, b) => a + b.durationMs, 0);
    const remRatio = remMs / params.actualSleepMs;
    if (remRatio < 0.15) score -= 10;
    else if (remRatio < 0.2) score -= 5;
  ...
  private averageTimeOfDay(timestamps: number[]): string {
    ...
    const avgMinutes = Math.round(minutesOfDay.reduce((a, b) => a + b, 0) / minutesOfDay.length) % (24 * 60);
  ...
  private generateRecommendations(analytics: {
    ...
    if (avgHours < 7) {
      recs.push(
        `You're averaging ${avgHours.toFixed(1)} hours — aim for 7-9 hours. Try going to bed 30 minutes earlier.`,
      );
    }
    ...
    if (analytics.avgEfficiency < 85) {
      recs.push(
        "Sleep efficiency is below 85%. Try not to stay in bed if you can't sleep — get up and do something relaxing.",
      );
    }
