 ...
 */
function determineTrainingStyle(plan: WorkoutPlan): 'strength' | 'hypertrophy' | 'endurance' | 'mixed' {
  const avgSets = plan.exercises.reduce((sum, e) => sum + e.sets, 0) / plan.exercises.length;
  const avgRepsLow =
    plan.exercises.reduce((sum, e) => {
 ...
 */
function determineStyleFromTypes(types: string[] = []): 'strength' | 'hypertrophy' | 'endurance' | 'mixed' {
 ...
 */
export async function getRecoveryOverviewUnified(userId: string = 'user_local_001'): Promise<{
