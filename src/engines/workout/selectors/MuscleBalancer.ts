...
export function analyzeBalance(
  ...
  const recommendedFocus: TargetMuscle[] = [
    ...underworked.slice(0, 3),
    ...imbalanced.map(i => i.weak).slice(0, 2),
  ];
 ...
 */
export function analyzePatternBalance(
 ...
 */
export function isPatternBalanced(
  balance: PatternBalance,
  minPatterns: number = 2
): boolean {
 ...
 */
export function suggestTrainingSplit(
  weeklyVolume: Map<TargetMuscle, number>,
...
export function calculateMuscleOverlap(
  exercise1: ExerciseWithDetails,
  exercise2: ExerciseWithDetails
): number {
 ...
 */
export function optimizeExerciseOrder(
  exercises: ExerciseWithDetails[]
): ExerciseWithDetails[] {
