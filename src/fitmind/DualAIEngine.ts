...
const COACH_INTENT_DEFS: Array<{ id: string; keywords: string[]; weight?: number }> = [
  ...
  { id: 'meal_prep', keywords: ['meal prep', 'meal plan', 'cook', 'recipe', 'breakfast', 'lunch', 'dinner', 'snack', 'what to eat', 'what should i eat'] },
  { id: 'macros', keywords: ['macro', 'how much protein', 'how many calories', 'calorie count', 'tdee', 'bulk', 'cut', 'caloric'] },
  { id: 'body_transform', keywords: ['transform', 'physique', 'appearance', 'aesthetic', 'craft my body', 'body transformation', 'recomp'] },
  { id: 'warmup', keywords: ['warm up', 'warmup', 'cool down', 'cooldown', 'before workout', 'after workout'] },
  { id: 'exercise_rec', keywords: ['recommend', 'suggest exercise', 'what exercise', 'which exercise', 'workout idea', 'exercise for', 'best exercise'] },
  { id: 'frequency', keywords: ['how often', 'frequency', 'schedule', 'routine', 'split', 'training days', 'days per week'] },
  { id: 'motivation', keywords: ['motivation', 'tired', "can't", 'give up', 'hard', 'push through', 'keep going', 'struggle', 'motivate', 'lazy', 'no energy', 'exhausted', 'quit'] },
  ...
  { id: 'progress', keywords: ['progress', 'results', 'improve', 'plateau', 'stuck', 'not seeing results', 'getting stronger'] },
  ...
  { id: 'injury', keywords: ['injury', 'prevent injury', 'joint pain', 'knee pain', 'shoulder pain', 'back pain', 'wrist pain', 'hurt'] },
  { id: 'supplements', keywords: ['supplement', 'creatine', 'protein powder', 'bcaa', 'pre workout', 'vitamin', 'fish oil'] },
  { id: 'mental', keywords: ['mental', 'stress', 'anxiety', 'confidence', 'discipline', 'mindset', 'mental health', 'depression'] },
  { id: 'hydration', keywords: ['water', 'hydration', 'drink water', 'dehydrated', 'thirsty', 'electrolyte'] },
  { id: 'weight', keywords: ['lose weight', 'gain weight', 'fat loss', 'skinny', 'overweight', 'bmi', 'body fat', 'weight loss'] },
  // Phase 4: New intent definitions
  { id: 'skill_progressions', keywords: ['progression', 'progress to', 'next level', 'advance', 'bodyweight skill', 'handstand', 'muscle up', 'planche', 'pistol squat', 'unlock'] },
  { id: 'outdoor', keywords: ['outdoor', 'outside', 'park', 'playground', 'fresh air', 'nature', 'garden'] },
  { id: 'plateau', keywords: ['plateau', 'stuck', 'stagnant', 'not progressing', 'no progress', 'same weight', 'same reps'] },
  ...
  { id: 'seasonal', keywords: ['summer', 'winter', 'spring', 'autumn', 'fall', 'hot weather', 'cold weather', 'rain', 'season'] },
  { id: 'energy', keywords: ['energy', 'low energy', 'pre workout', 'caffeine', 'tired before', 'no energy', 'fatigue before', 'boost'] },
  { id: 'breathing', keywords: ['breath', 'breathing', 'breathe', 'inhale', 'exhale', 'oxygen', 'winded', 'out of breath'] },
  { id: 'time_mgmt', keywords: ['no time', 'busy', 'short workout', 'quick workout', 'time efficient', '15 minutes', '20 minutes', 'fit in'] },
  { id: 'mindfulness', keywords: ['mindful', 'focus', 'concentrate', 'mind muscle', 'visualization', 'meditate', 'mental training', 'presence'] },
  { id: 'recovery_protocol', keywords: ['foam roll', 'ice bath', 'cold shower', 'sauna', 'massage', 'active recovery', 'recovery protocol', 'rest day routine'] },
];
...
export class DualAIEngine {
  ...
  private async processCoachQuery(input: string, context: AIContext): Promise<AIResponse> {
    ...
    if (intentScores.length === 0 && context.conversationHistory?.length) {
      const isFollowUp = /^(what about|tell me more|more on|more about|and |so |also |but |how about|yes|yeah|sure|go on|continue|okay |ok )/i.test(input.trim());
      if (isFollowUp) {
        const lastAssistant = [...context.conversationHistory].reverse()
          .find(m => m.role === 'assistant');
        if (lastAssistant) {
          const prevScores = this.scoreAllIntents(
            lastAssistant.content.toLowerCase().slice(0, 300)
          );
    ...
    switch (topIntent?.id) {
      ...
      case 'macros': {
        message = this.fillTemplate(
          this.pickRandomAvoidingRepeats(COACH_TEMPLATES.macros, 'coach_macros'),
          context
        );
        break;
      }
      ...
      case 'sleep': {
        message = this.fillTemplate(
          this.pickRandomAvoidingRepeats(COACH_TEMPLATES.sleep, 'coach_sleep'),
          context
        );
        break;
      }
      ...
      case 'thanks': {
        message = this.pickRandomAvoidingRepeats(
          (COACH_TEMPLATES as any).thanks, 'coach_thanks'
        );
        break;
      }
    ...
    if (intentScores.length > 1 && topIntent &&
        topIntent.id !== 'thanks' && topIntent.id !== 'greeting') {
  ...
  private async generateCoachResponse(input: string, context: AIContext): Promise<string> {
    ...
    const topicMap: Record<string, { bucket: string[]; category: string }> = {
      'workout|train|exercise|lift|gym|session': { bucket: (COACH_TEMPLATES as any).workout_motivation, category: 'coach_motivation' },
      ...
      'tired|exhausted|fatigued|worn out': { bucket: (COACH_TEMPLATES as any).fatigue_warning, category: 'coach_fatigue' },
      'muscle|strong|strength|gains': { bucket: (COACH_TEMPLATES as any).progressive_overload, category: 'coach_overload' },
      'lose|fat|lean|cut|slim|tone': { bucket: (COACH_TEMPLATES as any).weight_management, category: 'coach_weight' },
      'run|cardio|jog|sprint|endurance': { bucket: (COACH_TEMPLATES as any).sport_specific.runner, category: 'coach_sport' },
    ...
    if (matchedTemplate) {
      ...
      if (context.workoutContext?.currentExercise) {
        ...
        const formTips = COACH_TEMPLATES.form_tips[muscleGroup as keyof typeof COACH_TEMPLATES.form_tips] || COACH_TEMPLATES.form_tips.core;
        parts.push(this.fillTemplate(
          this.pickRandomAvoidingRepeats(formTips, `coach_form_${muscleGroup}`),
          context
        ));
        ...
        parts.push(`Your ${context.userProfile.streakDays}-day streak shows real commitment. That consistency is what separates results from wishes.`);
      }
    }
    ...
    if (memory) {
      if (memory.recentTopics.length > 0 && !matchedTemplate) {
        ...
        parts.push(`We've been covering ${topics} — want me to go deeper on any of those, or is there something new on your mind?`);
      }
      if (memory.mentionedExercises.length > 0 && Math.random() > 0.7) {
        parts.push(`By the way, since you've been working on ${memory.mentionedExercises[0]} — keep at it, consistency is key!`);
      }
    }
