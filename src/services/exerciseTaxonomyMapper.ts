...
export function mapMuscles(externalMuscles: string[] | null | undefined): TargetMuscle[] {
  ...
  return externalMuscles
    .map(m => mapMuscle(m))
    .filter((m): m is TargetMuscle => m !== null);
}
...
export function inferEquipmentLevel(equipment: EquipmentItem | null): EquipmentLevel {
  ...
  const playgroundEquipment: EquipmentItem[] = [
    'pull_up_bar', 'parallel_bars', 'monkey_bars', 'bench', 
    'hill', 'sand', 'sled', 'parachute', 'parallettes', 'rings'
  ];
...
export function inferSpaceRequired(
  category: string | null | undefined,
  name: string
): SpaceFilter {
  const cat = (category || '').toLowerCase();
