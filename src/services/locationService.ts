...
export function getDefaultLocation(): UserLocation {
  ...
  try {
    ...
    if (
      tzLower.includes('africa/johannesburg') ||
      tzLower.includes('africa/harare') ||
      tzLower.includes('africa/maputo')
    ) {
      return {
        latitude: -26.2,
        longitude: 28.0,
    ...
    if (
      tzLower.includes('america/new_york') ||
      tzLower.includes('america/chicago') ||
      tzLower.includes('america/denver') ||
      tzLower.includes('america/los_angeles')
    ) {
      return {
        latitude: 40.7,
        longitude: -74.0,
...
const GLOBAL_FOODS: FoodItem[] = [
  // Universal foods available everywhere
  {
    name: 'Eggs',
    category: 'protein',
    description: 'Whole eggs, scrambled, boiled, or fried',
  ...
  },
  {
    name: 'Chicken Breast',
    category: 'protein',
    description: 'Grilled or baked chicken breast',
  ...
  },
  {
    name: 'Oats',
    category: 'carb',
    description: 'Rolled oats or oatmeal',
  ...
  },
  {
    name: 'Spinach',
    category: 'vegetable',
    description: 'Fresh or cooked spinach',
  ...
  },
  {
    name: 'Sweet Potato',
    category: 'carb',
    description: 'Baked or boiled sweet potato',
  ...
  },
  {
    name: 'Almonds',
    category: 'fat',
    description: 'Raw or roasted almonds',
  ...
  },
  {
    name: 'Broccoli',
    category: 'vegetable',
    description: 'Steamed or raw broccoli',
  ...
  },
  {
    name: 'Salmon',
    category: 'protein',
    description: 'Grilled or baked salmon fillet',
  ...
  },
  {
    name: 'Lentils',
    category: 'protein',
    description: 'Cooked lentils (any variety)',
  ...
  },
  {
    ...
    category: 'carb',
    description: 'Whole grain bread slices',
...
const MEAL_SUGGESTIONS_CONFIG: Record<string, { title: string; categories: FoodItem['category'][]; tip: string }> = {
  breakfast: {
    title: '🍳 Breakfast',
    categories: ['protein', 'carb', 'fruit'],
    tip: 'Eat within 1 hour of waking for best energy',
  },
  lunch: {
    title: '🥗 Lunch',
    categories: ['protein', 'carb', 'vegetable'],
    tip: 'Balance protein with complex carbs for sustained energy',
  },
  dinner: {
    title: '🍗 Dinner',
    categories: ['protein', 'vegetable', 'fat'],
    tip: 'Focus on lean protein and veggies for recovery',
  },
  'pre-workout': {
    title: '⚡ Pre-Workout',
    categories: ['carb', 'fruit'],
    tip: 'Light carbs 30-60 min before training',
  },
  'post-workout': {
    title: '💪 Post-Workout',
    categories: ['protein', 'carb'],
    tip: 'Consume protein within 30 min post-exercise',
  },
  snack: {
    title: '🥜 Snack',
    categories: ['fat', 'fruit', 'snack'],
    tip: 'Keep portions small — aim for 150-200 calories',
  },
};
...
function partitionFoodsAcrossMeals(foods: FoodItem[]): Map<string, FoodItem[]> {
  ...
  for (const food of foods) {
    const eligibleMeals = MEAL_ORDER.filter((m) => MEAL_SUGGESTIONS_CONFIG[m]?.categories.includes(food.category));
